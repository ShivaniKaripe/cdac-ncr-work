...
void wmain(void)
{
	...
	StringCbCopy((LPSTR)((char*)pSessionProperties + pSessionProperties->LogFileNameOffset), sizeof(LOGFILE_PATH),(LPCSTR)LOGFILE_PATH);

	// Create the trace session.

	status = StartTrace((PTRACEHANDLE)&SessionHandle, NULL, pSessionProperties);
...
cleanup:
	...
	if (SessionHandle)
	{
		...
		{
			wprintf(L"succes in control trace");
		}
	}
